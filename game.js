...
function shootBullet() {
  ...
  const bullet = {
    position: playerAvatar.position.clone().add(new THREE.Vector3(0, 2, 0)),
...
function updateBullets() {
  for (let i = bullets.length - 1; i >= 0; i--) {
    ...
    if (bullet.glowMesh) glowMesh.position.copy(bullet.position);
    
    checkBulletCollisions(bullet, i);
    
    if (bullet.position.distanceTo(playerAvatar.position) > 1000) {
      bullet.active = false;
    }
  }
}
...
function checkBulletCollisions(bullet, bulletIndex) {
  ...
  for (let i = 0; i < nftObjects.length; i++) {
    ...
    if (nftBox.containsPoint(bullet.position)) {
      console.log("NFT hit! Owner would get more bullets");
    }
  }
}
